...
<div style="max-width:500px">
    <div style="font-size:18px;font-weight:500;margin-bottom:20px">Modifier le produit</div>
    ...
    <div style="background:#fff;border-radius:12px;border:0.5px solid #e0e0d8;padding:24px">
        <form action="{{ route('products.update', $product) }}" method="POST">
            ...
            <div style="margin-bottom:16px">
                ...
                <input type="text" name="nom" value="{{ old('nom', $product->nom) }}" style="width:100%;padding:10px 12px;border:0.5px solid #d3d1c7;border-radius:8px;font-size:14px;outline:none" required>
            ...
            <div style="margin-bottom:16px">
                ...
                <input type="number" name="prix_achat" value="{{ old('prix_achat', $product->prix_achat) }}" step="0.01" min="0" style="width:100%;padding:10px 12px;border:0.5px solid #d3d1c7;border-radius:8px;font-size:14px;outline:none" required>
            ...
            <div style="margin-bottom:24px">
                ...
                <input type="number" name="prix_vente" value="{{ old('prix_vente', $product->prix_vente) }}" step="0.01" min="0" style="width:100%;padding:10px 12px;border:0.5px solid #d3d1c7;border-radius:8px;font-size:14px;outline:none" required>
                @error('prix_vente')<div style="color:#A32D2D;font-size:12px;margin-top:4px">{{ $message }}</div>@enderror
            </div>

            <div style="display:flex;gap:10px">
                <button type="submit" style="background:#185FA5;color:#fff;padding:10px 20px;border:none;border-radius:8px;font-size:14px;cursor:pointer">Mettre à jour</button>
